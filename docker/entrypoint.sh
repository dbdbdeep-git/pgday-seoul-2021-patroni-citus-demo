...
dnsmasq --user=root --conf-file=/etc/dnsmasq.d/10-consul -8 /var/log/dnsmasq.log
...
#        exec "$@" -advertise $DOCKER_IP
        exec consul agent -advertise $DOCKER_IP -config-file=/etc/consul.d/consul.json
        ;;
    etcd)
    ...
    zookeeper)
        ...
        ;;
    postgres)
        echo "PostgreSQL Server instance..."
        ;;
esac
...
#    consul agent -advertise $DOCKER_IP -client 0.0.0.0 -retry-join consul_1 -retry-join consul_2 -retry-join consul_3 -ui -data-dir=/tmp/consul > /var/log/consul.log 2> /var/log/consul.err &
    consul agent -advertise $DOCKER_IP --node=$HOSTNAME --node-id=$(uuidgen) -retry-join consul-1 -retry-join consul-2 -retry-join consul-3 -data-dir=/tmp/consul > /var/log/consul.log 2> /var/log/consul.err &
fi
